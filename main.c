...
int main(int argc, char* argv[])
{
    ...
    printf(CONSOLE_WHITE "Press Y For the " CONSOLE_GREEN " Sauce" CONSOLE_WHITE " :)  \n"CONSOLE_YELLOW); 
    ...
    while (appletMainLoop())
    {
        ...
        if (kDown & KEY_PLUS)
         ...
         if (kDown & KEY_Y)
                     {
             srand(time(NULL));
             printf("www.nhentai.net/g/%d\n", rand() % 600000 +  100000);
             }
        if (kDown & KEY_X)
            {
            showKeyboard(url, "Choose URL", "Enter a URL", "Go", "https://nhentai.net/g/");
             Result rc;
